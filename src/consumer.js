 ...
 */
class PulseQueue {
  constructor({client, bindings, handleMessage, ...options}) {
    this.client = client;
    ...
    // set up to call _handleConnection on new connections and, if there
  ...
  async _handleConnection(conn) {
    try {
      ...
      const exclusive = !this.options.queueName;
      const queueName = this.client.fullObjectName(
        'queue',
        this.options.queueName || `exclusive/${slugid.v4()}`);
      ...
      this._queueBound && this._queueBound(); // (hook for tests)
