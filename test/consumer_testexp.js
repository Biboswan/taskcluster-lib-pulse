const {consume} = require('../src');
const amqplib = require('amqplib');
const {ConnectionManager} = require('amqplib-mocks');
...
suite('PulseQueue', function() {
  ...
  const publishFakeMessages = async (amqpconn) => {
    const chan = await amqpconn.createChannel();
  ...
  test('consume messages', async function() {
    ...
    const amqpconn = await connmanager.connect('amqp://mockserver');// connection used for publish and consume
    const chan = await amqpconn.createChannel();
    await chan.assertExchange(exchangeName, 'topic');
    ...
    await new Promise(async (resolve, reject) => {
      try {
        ...
        await publishFakeMessages(amqpconn);
      } catch (err) {
        reject(err);
      }
    });
